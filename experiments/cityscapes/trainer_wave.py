...
def find_worst_task(flat_grads, flat_mean):
    """Return index of the task with lowest cosine similarity to the mean gradient."""
    sims = torch.stack([
        F.cosine_similarity(g, flat_mean, dim=0) for g in flat_grads
    ])
    worst_idx = int(torch.argmin(sims).item())
    return worst_idx, sims

def spectral_perturbation_wave(param, clean_grad, worst_residual, rho, lam):
    """
    WAVE spectral perturbation for one layer.

    Returns ε_W = ρ ‖W‖_F [ λ · NS(R_W) + (1−λ) · NS(Ḡ_W) ]

    where R_W = G_{k*,W} − Ḡ_W  is the worst-task residual.
    """
    g = clean_grad
    if g is None:
        return None
    ...
    use_disagreement = (
        ...
        and worst_residual.shape == orig_shape
    )
    if use_disagreement:
        R_2d = worst_residual.view(worst_residual.size(0), -1) if worst_residual.ndim > 2 else worst_residual
        if transposed:
            R_2d = R_2d.T
        if R_2d.norm() > 1e-12:
            Q_D = zeropower_via_newtonschulz5(R_2d)
        else:
            Q_D = Q
            lam = 0.0
    else:
...
def main(args, device):
    ...
    for epoch in epoch_iter:
        ...
        for j, batch in enumerate(train_loader):
            ...
            for i, loss_i in enumerate(losses):
                ...
                for k, g in enumerate(grad_i):
                    if g is not None:
                        ...
                        task_grads_per_param[k].append(None)

            # ==========================================================
            # Phase 2: Similarity, λ, worst-task residual
            # ==========================================================
            ...
            flat_mean = torch.cat([g.flatten() for g in clean_grad_w])
            worst_idx, all_sims = find_worst_task(task_grads_flattened, flat_mean)
            worst_sim_val = all_sims[worst_idx].item()

            worst_residuals = []
            for k in range(n_params):
                g_worst = task_grads_per_param[k][worst_idx]
                if g_worst is None:
                    worst_residuals.append(None)
                else:
                    worst_residuals.append(g_worst - clean_grad_w[k])

            del task_grads_per_param, task_grads_flattened

            # ==========================================================
            # Phase 3: WAVE Spectral Perturbations
            # ==========================================================
            params_list = list(model.parameters())
            eps = [
                spectral_perturbation_wave(
                    param=p,
                    clean_grad=g,
                    worst_residual=r,
                    rho=args.rho,
                ...
                for p, g, r in zip(params_list, clean_grad_w, worst_residuals)
            ]
            ...
            epoch_iter.set_description(
                ...
                f"sim: {sim_val:.3f}, lambda: {lam_val:.3f}, worst_task: {worst_idx}")
        ...
        with torch.no_grad():
            ...
            if wandb.run is not None:
                ...
                wandb.log({"Train Worst Task Sim": worst_sim_val}, step=epoch)
            ...
            name = f"{args.method}_rho{args.rho}_lmax{args.lambda_max}_sd{args.seed}_wave"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--lambda_max",
                        type=float,
                        default=0.8,
                        help="Maximum mixing weight toward worst-task perturbation.")
