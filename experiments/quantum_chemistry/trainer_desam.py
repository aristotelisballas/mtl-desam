...
from experiments.quantum_chemistry.utils import GradEstimator
...
def main(
    ...
    for epoch in epoch_iterator:
        ...
        for j, data in enumerate(train_loader):
            ...
            # scaled_losses = losses / losses.detach()
            # scaled_losses.mean().backward()
            # Get the estimated gradient
            zeroth_grads = {}
            for task in range(n_tasks):
                zeroth_grads[task] = GradEstimator(model, eps=zo_eps).forward(
                    data, task)
            ...
            for task in range(n_tasks):
                ...
                for n, p in model.named_parameters():
                    if n.startswith(f"head_{task}"):
                        ...
                        if p.grad is not None:
                            ...
                            # DESAM SCALE
                            g = p.grad
                            if desam and n in desam_scales:
                                g = g * desam_scales[n]
                                
                            task_norms[n] = (
                                (torch.abs(p) if adaptive else 1.0) *
                                g).norm(p=2).data.clone()
                            task_ep = ((torch.pow(p, 2) if adaptive else 1.0) *
                                       g).data.clone()
                        task_epsilon_params[task][n] = task_ep

                # task_norm = torch.norm(torch.stack(list(task_norms.values())),
                #                        p=2)
                # task_scale = (rho / (task_norm + 1e-12)).item()
                ...
                for n, p in model.named_parameters():
                    ...
                    if p.grad is not None:
                        ...
                        g = (1 - beta) * p.grad.data.clone() + beta * n1 * (
                            zeroth_grads[task][n] / n2)
                        ...
                        shared_norms[n] = (
                            (torch.abs(p) if adaptive else 1.0) *
                            g).norm(p=2).data.clone()
                        shared_ep = ((torch.pow(p, 2) if adaptive else 1.0) *
                                     g).data.clone()
                ...
                shared_norm = torch.norm(torch.stack(
                    list(shared_norms.values())),
                                         p=2)
                shared_scale = (rho / (shared_norm + 1e-12)).item()
