...
def main(args, device):
    ...
    for epoch in epoch_iter:
        ...
        for j, batch in enumerate(train_loader):
            ...
            zeroth_grads = {}
            targets_map = {0: train_label, 1: train_depth, 2: train_normal}
            for task in range(n_tasks):
                zeroth_grads[task] = GradEstimator(
                    model, eps=args.zo_eps).forward(train_data, targets_map[task], task)
            ...
            for task in range(n_tasks):
                ...
                if task == 0:
                    ...
                    calc_loss(train_pred_pert[2], train_normal, "normal").backward()
                else:
