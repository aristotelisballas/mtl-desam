...
def calculate_gradient_variance(grads_list, measure="normalized_std", eps=1e-12):
    """
    Dispersion of the domain/task gradients, used as the noise scale.

    The variance across domains is the trace of the gradient covariance,
        V = (1/T) * sum_t ||g_t - g_mean||^2,
    i.e. the mean squared deviation of each domain gradient from the consensus
    gradient. It is 0 when all domains agree and grows as they disagree, both in
    direction and in magnitude (unlike cosine similarity, which only sees
    direction).

    Since V carries the (arbitrary) scale of the loss/gradients, the normalized
    variants divide by the consensus gradient, giving a dimensionless quantity:
        normalized_var = V / ||g_mean||^2
        normalized_std = sqrt(V) / ||g_mean||     (coefficient of variation)
    """
    if len(grads_list) < 2:
        return torch.zeros((), device=grads_list[0].device)

    n_domains = len(grads_list)

    # Consensus (mean) gradient, accumulated in-place to avoid stacking all
    # flattened domain gradients into one extra tensor.
    mean_grad = torch.zeros_like(grads_list[0])
    for g in grads_list:
        mean_grad += g
    mean_grad /= n_domains

    var = torch.zeros((), device=mean_grad.device, dtype=mean_grad.dtype)
    for g in grads_list:
        var += (g - mean_grad).pow(2).sum()
    var /= n_domains
    ...
    mean_sq_norm = mean_grad.pow(2).sum()
    if measure == "normalized_var":
        return var / (mean_sq_norm + eps)
    # default: normalized_std
    return var.clamp(min=0.0).sqrt() / (mean_sq_norm.sqrt() + eps)
...
def main(args, device):
    ...
    for epoch in epoch_iter:
        ...
        for j, batch in enumerate(train_loader):
            ...
            # ==========================================================
            # Phase 2: Domain Gradient Variance & Noise Scale Calculation
            # ==========================================================
            grad_var = calculate_gradient_variance(task_grads_flattened,
                                                   measure=args.var_measure)
            if args.var_clip > 0:
                grad_var = grad_var.clamp(max=args.var_clip)
            alpha = args.var_gamma * grad_var
            ...
            # ==========================================================
            # Phase 6: Restore Weights & Inject Variance Noise (Descent Step)
            # ==========================================================
            with torch.no_grad():
                for p, v in zip(model.parameters(), eps):
                    if v is not None:
                    ...
                    # Inject noise scaled by the across-domain gradient variance
            ...
            var_val = grad_var.item() if isinstance(grad_var, torch.Tensor) else grad_var
            ...
            epoch_iter.set_description(
                ...
                f"grad var: {var_val:.3f}, alpha: {alpha_val:.5f}")
        ...
        with torch.no_grad():  # operations inside don't track history
            ...
            if wandb.run is not None:
                wandb.log({"Train Gradient Variance": var_val}, step=epoch)
            ...
            name = f"{args.method}_rho{args.rho}_gamma{args.var_gamma}_{args.var_measure}_sd{args.seed}_sagevar"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--rho",
                        type=float,
                        default=0.0005,
                        help="Rho for pertubation in MuonSAM.")
    parser.add_argument("--var_gamma",
                        "--gga_l_gamma",
                        type=float,
                        default=0.0001,
                        help="Gamma scalar for the gradient-variance noise.")
    parser.add_argument(
        "--var_measure",
        type=str,
        default="normalized_std",
        choices=["normalized_std", "normalized_var", "raw_std", "raw_var"],
        help="Dispersion statistic used as the noise scale. The normalized "
        "variants divide by the consensus gradient norm and are therefore "
        "invariant to the overall gradient magnitude.")
    parser.add_argument("--var_clip",
                        type=float,
                        default=0.0,
                        help="Upper clip on the dispersion before scaling by "
                        "gamma (<=0 disables clipping).")
